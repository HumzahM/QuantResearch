...
def get_event_month_blocks_volume(window_size):
    ...
    data["total daily volume"].fillna(value=0)
    data = data[data["total daily volume"] > 1]

    data['month'] = data['date'].dt.strftime('%Y-%m')
    data['year'] = data['date'].dt.strftime('%Y')
    data['monthly avg trade'] = data.groupby("month")['total daily volume'].transform('mean')

    # Applying the function to a single column of the DataFrame
    data['trading moving average'] = np.exp(symmetric_smoothing(np.log(data['total daily volume']), window_size, method='average'))
    data['trading moving average no log'] = symmetric_smoothing(data['total daily volume'], window_size, method='average')
    ...
    plt.title(STOCK_NAME + " number daily volume" + str(window_size) + " window size ")
    plt.legend()
    plt.figure()

    normalized_trading = (y_data / y_model).to_numpy()

    trading_moving_average_with_log = (data['total daily volume'] / data['trading moving average']).to_numpy()

    trading_moving_average_with_log_without_log = (data['total daily volume'] / data['trading moving average no log']).to_numpy()
    ...
    num_events = len(data.groupby('month')['total daily volume'].sum())
