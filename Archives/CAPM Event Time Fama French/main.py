...
def calculate_betas(monthly_returns, fama_french):
    # Prepare the DataFrame to collect beta values
    results = []
    ...
    for permco in tqdm(unique_permcos):
        ...
        for data_type in [1, 2]:
            ...
            continuous_blocks_indices = np.split(data_range.index, np.where(np.diff(data_range['sequence #']) != 1)[0] + 1)
            
            for indices in continuous_blocks_indices:
                if len(indices) >= 60:
                    for i in range(60, len(indices)):
                        block = data_range.loc[indices[i-60:i]]
                        y = block['equity_returns']
                        X = block[[f'market_return_range{data_type}', f'smb_return_range{data_type}', f'hml_return_range{data_type}']]
                        
                        betas = calculate_multi_beta(y, X)
                        
                        # Prepare a dictionary for the row
                        row = {
                            'sequence #': block['sequence #'].iloc[-1],  # Example of unique identifier
                            'market_beta': betas[0],
                            'SMB_beta': betas[1],
                            'HML_beta': betas[2],
                            'equity_returns': block['equity_returns'].iloc[-1],
                            'market_cap': block['market_cap'].iloc[-1],
                            'type': data_type
                        }
                        results.append(row)
    
    # Convert results to a DataFrame
    betad_monthly_returns = pd.DataFrame(results)
    # Optionally, merge back on 'sequence #' or another key to combine with original data
    # This step depends on whether you need to join these results with your original DataFrame
    print(betad_monthly_returns)
    return betad_monthly_returns
...
def calculate_price_of_betas(monthly_returns):
    ...
    print(ttest_ind(price_of_market_beta_type1, price_of_market_beta_type2))
    print(ttest_ind(price_of_SMB_beta_type1, price_of_SMB_beta_type2))
    print(ttest_ind(price_of_HML_beta_type1, price_of_HML_beta_type2))

    plt.figure()
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(6, 12))
    _, bins, _ = ax1.hist(price_of_market_beta_type1, bins=20, alpha=0.5, histtype=u'step', label='Calendar')
    ax1.hist(price_of_market_beta_type2, bins=bins, alpha=0.5, histtype=u'step', label='Event')
    ax1.set_title('Price of Market Beta')
    ax1.set_xlim(-0.25, 0.25)
    ax1.legend()

    _, bins, _ = ax2.hist(price_of_SMB_beta_type1, bins=20, alpha=0.5, histtype=u'step', label='Calendar')
    ax2.hist(price_of_SMB_beta_type2, bins=bins, alpha=0.5, histtype=u'step', label='Event')
    ax2.set_xlim(-0.25, 0.25)
    ax2.set_title('Price of SMB Beta')
    ax2.legend()

    _, bins, _ = ax3.hist(price_of_HML_beta_type1, bins=20, alpha=0.5, histtype=u'step', label='Calendar')
    ax3.hist(price_of_HML_beta_type2, bins=bins, alpha=0.5, histtype=u'step', label='Event')
...
try:
    if rerunMonthlyReturns or not (os.path.exists(monthly_returns_filename)):
        ...
        monthly_returns = calculate_monthly_returns(stocks, monthly_day_ranges, event_month_ranges, calculate_sp=False)
