...
def fetch_stock_data(start_date, end_date):
    ...
    data['NUMTRD'] = data['NUMTRD'].fillna(1)
    print(data)
...
def get_event_month_blocks(data, window_size):
    ...
    data['trading moving average'] = symmetric_smoothing(data['NUMTRD'], window_size, method='average')
    data = data[(data['date'].dt.year > data['date'].dt.year.min() + 1) & (data['date'].dt.year < data['date'].dt.year.max() - 1)].reset_index(drop=True)
    if data.empty:
...
def process_data(data, rf, window_size):
    ...
    for stock in unique_permcos:
        ...
        rf_filtered = rf[(rf['date'] >= stock_data['date'].min()) & (rf['date'] <= stock_data['date'].max())]   
        stock_data = stock_data.merge(rf_filtered, left_on='date', right_on='date')
        ...
        for i, ((start1, end1), (start2, end2)) in enumerate(zip(event_months, months)):
            if start1 >= first_date_stock and end1 <= last_date_stock and start2 >= first_date_stock and end2 <= last_date_stock:
                filtered_stock_data1 = stock_data[(stock_data['date'] >= start1) & (stock_data['date'] <= end1)]
                filtered_stock_data2 = stock_data[(stock_data['date'] >= start2) & (stock_data['date'] <= end2)]

                monthly_return1 = filtered_stock_data1['log returns'].sum()
                monthly_return2 = filtered_stock_data2['log returns'].sum()
...
process_data(data, rf=risk_free, window_size=1512)
